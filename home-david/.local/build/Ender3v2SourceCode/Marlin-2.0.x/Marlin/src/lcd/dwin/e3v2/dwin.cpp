...
#define DWIN_FONT_STAT font8x16
...
constexpr uint16_t TROWS = 6, MROWS = TROWS - 1,        // Total rows, and other-than-Back
                   ...
                   MENU_CHR_W = 8, STAT_CHR_W = 8,
                   STAT_X_HOTEND_TMP = 33, STAT_Y_HOTEND_TMP = 382,
                   STAT_X_HOTEND_TRG = 33 + 4*STAT_CHR_W, STAT_Y_HOTEND_TRG = 382,
                   STAT_X_BED_TMP = 178, STAT_Y_BED_TMP = 382,
                   STAT_X_BED_TRG = 178 + 3*STAT_CHR_W, STAT_Y_BED_TRG = 382;
...
void ICON_Print() {
  if (select_page.now == 0) {
    ...
    DWIN_Draw_Rectangle(0, Icon_Frame, 17, 130, 126, 229);
    if (HMI_IsChinese())
...
void ICON_Prepare() {
  if (select_page.now == 1) {
    ...
    DWIN_Draw_Rectangle(0, Icon_Frame, 145, 130, 254, 229);
    if (HMI_IsChinese())
...
void ICON_Control() {
  if (select_page.now == 2) {
    ...
    DWIN_Draw_Rectangle(0, Icon_Frame, 17, 246, 126, 345);
    if (HMI_IsChinese())
...
void ICON_StartInfo(bool show) {
  if (show) {
    ...
    DWIN_Draw_Rectangle(0, Icon_Frame, 145, 246, 254, 345);
    if (HMI_IsChinese())
...
void ICON_Tune() {
  if (select_print.now == 0) {
    ...
    DWIN_Draw_Rectangle(0, Icon_Frame, 8, 252, 87, 351);
    if (HMI_IsChinese())
...
void ICON_Pause() {
  if (select_print.now == 1) {
    ...
    DWIN_Draw_Rectangle(0, Icon_Frame, 96, 252, 175, 351);
    if (HMI_IsChinese())
...
void ICON_Stop() {
  if (select_print.now == 2) {
    ...
    DWIN_Draw_Rectangle(0, Icon_Frame, 184, 252, 263, 351);
    if (HMI_IsChinese())
...
inline void Draw_Menu_Cursor(const uint8_t line) {
  ...
  /* DWDchg: CURSOR SHAPE IS FRAME
     =============================
     V SubMenu-jih je bil kurzoz prikazan kot okvirček na levi strani, 
     sedah je okvirček čez celotno besedilo v sub-menuju.
     Tako je bolj sistematično z Main Menu-jem. 
  */
  DWIN_Draw_Rectangle(0, Rectangle_Color, 10, MBASE(line) - 10 , 262, MBASE(line) + 25);
}
...
inline void Erase_Menu_Cursor(const uint8_t line) {
  ...
  /* DWDchg: CURSOR SHAPE IS FRAME
     =============================
     Da zbrišemo okvirček kurzprja.
  */
  DWIN_Draw_Rectangle(0, Color_Bg_Black, 10, MBASE(line) - 10 , 262, MBASE(line) + 25);
}
...
void update_variable() {
  ...
  /* Bottom temperature update */
    ...
    if (last_temp_hotend_current != thermalManager.temp_hotend[0].celsius) {
      DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, Color_White, Color_Bg_Black, 3, STAT_X_HOTEND_TMP, STAT_Y_HOTEND_TMP, thermalManager.temp_hotend[0].celsius);
      last_temp_hotend_current = thermalManager.temp_hotend[0].celsius;
    }
    if (last_temp_hotend_target != thermalManager.temp_hotend[0].target) {
      DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, Color_White, Color_Bg_Black, 3, STAT_X_HOTEND_TRG, STAT_Y_HOTEND_TRG, thermalManager.temp_hotend[0].target);
      last_temp_hotend_target = thermalManager.temp_hotend[0].target;
    }
  #endif
  #if HAS_HEATED_BED
    if (last_temp_bed_current != thermalManager.temp_bed.celsius) {
      DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, Color_White, Color_Bg_Black, 2, STAT_X_BED_TMP, STAT_Y_BED_TMP, thermalManager.temp_bed.celsius);
      last_temp_bed_current = thermalManager.temp_bed.celsius;
    }
    if (last_temp_bed_target != thermalManager.temp_bed.target) {
      DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, Color_White, Color_Bg_Black, 2, STAT_X_BED_TRG, STAT_Y_BED_TRG, thermalManager.temp_bed.target);
...
void Draw_Status_Area(const bool with_update) {
  ...
  //
    ...
    DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, Color_White, Color_Bg_Black, 3, STAT_X_HOTEND_TMP, STAT_Y_HOTEND_TMP, thermalManager.temp_hotend[0].celsius);
    DWIN_Draw_String(false, false, DWIN_FONT_STAT, Color_SkyBlue, Color_Bg_Black, STAT_X_HOTEND_TMP + 3*STAT_CHR_W, STAT_Y_HOTEND_TMP, F("["));
    DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, Color_White, Color_Bg_Black, 3, STAT_X_HOTEND_TRG, STAT_Y_HOTEND_TRG, thermalManager.temp_hotend[0].target);
    DWIN_Draw_String(false, false, DWIN_FONT_STAT, Color_SkyBlue, Color_Bg_Black, STAT_X_HOTEND_TRG + 3*STAT_CHR_W, STAT_Y_HOTEND_TRG, F("]"));
    DWIN_Draw_String(false, false, DWIN_FONT_STAT, Color_White, Color_Bg_Black, STAT_X_HOTEND_TRG + 5*STAT_CHR_W, STAT_Y_HOTEND_TRG, F("C"));
    DWIN_Draw_Rectangle(0, Color_White, STAT_X_HOTEND_TRG + 4.5*STAT_CHR_W, STAT_Y_HOTEND_TRG, STAT_X_HOTEND_TRG + 4.5*STAT_CHR_W+3, STAT_Y_HOTEND_TRG+3);
  #endif
  #if HOTENDS > 1
    // DWIN_ICON_Show(ICON,ICON_HotendTemp, 13, 381);
  #endif
    ...
    DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, Color_White, Color_Bg_Black, 2, STAT_X_BED_TMP, STAT_Y_BED_TMP, thermalManager.temp_bed.celsius);
    DWIN_Draw_String(false, false, DWIN_FONT_STAT, Color_SkyBlue, Color_Bg_Black, STAT_X_BED_TMP + 2*STAT_CHR_W, STAT_Y_BED_TMP, F("["));
    DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, Color_White, Color_Bg_Black, 2, STAT_X_BED_TRG, STAT_Y_BED_TRG, thermalManager.temp_bed.target);
    DWIN_Draw_String(false, false, DWIN_FONT_STAT, Color_SkyBlue, Color_Bg_Black, STAT_X_BED_TRG + 2*STAT_CHR_W, STAT_Y_HOTEND_TRG, F("]"));
    DWIN_Draw_String(false, false, DWIN_FONT_STAT, Color_White, Color_Bg_Black, STAT_X_BED_TRG + 4*STAT_CHR_W, STAT_Y_BED_TRG, F("C"));
    DWIN_Draw_Rectangle(0, Color_White, STAT_X_BED_TRG + 3.5*STAT_CHR_W, STAT_Y_BED_TRG, STAT_X_BED_TRG + 3.5*STAT_CHR_W+3, STAT_Y_BED_TRG+3);
  #endif
