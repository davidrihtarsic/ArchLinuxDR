...
#notify-send "UDISKSCTL :" "$($(lsblk | grep "sd[c-z]\|mmc"|sed 's/^../s/' |dmenu -l 10 -p "(UN-)MOUNT USB" | awk '{if($7 == ""){print "udisksctl mount -b /dev/"$1}else{print "udisksctl unmount -b /dev/"$1}}'))"
